...
if __name__ == "__main__":
    ...
    if args.load_model:
        ...
        t = time.time()
        fine_tuning_loss_values = np.zeros(args.con_epochs, dtype=np.float)
    ...
    # con_epochs = np.array([100, 150, 200, 250, 300, 350, 400, 450, 500], dtype=np.int)
    #
    # batch_size = args.batch_size
    # learning_rate = args.learning_rate
    # for epoch_idx in range(con_epochs.shape[0]):
    #     args.con_epochs = con_epochs[epoch_idx]
    #
    #     set_seed(args.seed)
    #     device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    #     mv_data = MultiviewData(args.db, device)
    ...
    #         f.write(
    #             '{} \t {} \t {} \t {} \t {} \t {:.4f} \t {:.3f} \t {:.3f} \t {:.6f} \t {:.6f} \t {:.6f} \t {:.6f} '
    #             '\t {:.4f} \n'.format(epoch_idx, args.con_epochs, dim_high_feature, dim_low_feature, batch_size,
    ...
    # lambdas = np.array([0.001, 0.005, 0.01, 0.05], dtype=np.float32)
    # betas = np.array([0.001, 0.005, 0.01, 0.05], dtype=np.float32)
    #
    # batch_size = args.batch_size
    # learning_rate = args.learning_rate
    ...
    #             f.write(
    #                 '{} \t {} \t {} \t {} \t {} \t {:.4f} \t {:.3f} \t {:.3f} \t {:.6f} \t {:.6f} \t {:.6f} \t {:.6f} '
    #                 '\t {:.4f} \n'.format(lmd_idx, beta_idx, dim_high_feature, dim_low_feature, batch_size,
    #                                       learning_rate, lmd, beta, acc, nmi, pur, ari, (time.time() - t)))
    #             f.flush()
